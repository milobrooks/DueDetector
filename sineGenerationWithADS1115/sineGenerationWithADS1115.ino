...
*/
//if(peat > 32){//timeer + 1000 <= millis()) {
 int i = 0;
 while(Serial.availableForWrite() > 3){
  /* Serial.write((holding[i] >> 8) & 0xFF);
   Serial.write(holding[i] & 0xFF);
   Serial.write(0xA); */
...
holding[peat] = ads.getLastConversionResults();
peat++;
}
