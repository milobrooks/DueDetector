...
char sendOverSerial[8];
String serialString;
short serialStringLength;
bool stringComplete;

unsigned int lastSend;
unsigned int sendInterval = 100;//100//20; //change type if more than 250
int adc[3];
unsigned int readHoldingAr [50][3];
unsigned int readADS;
unsigned int readInterval = 10;//20;
int averagingBuffer = 0;
...
void loop() {
  if (readADS + readInterval < millis()) {
    readHoldingAr[averagingBuffer][0] = ads.readADC_SingleEnded(0);
    readHoldingAr[averagingBuffer][1] = ads.readADC_SingleEnded(1);
    readHoldingAr[averagingBuffer][2] = ads.readADC_SingleEnded(2);
    averagingBuffer++;
    readADS = millis();
  }


  if (sendInterval + lastSend < millis()) {


    for (int p = 0; p < 3 ; p++) {

      for (int a = 0; a < averagingBuffer; a++) {
        adc[p] += readHoldingAr[a][p];
        readHoldingAr[a][p] = 0 ;
      }

      adc[p] = adc[p] / averagingBuffer;
      char pos[2];
      String poi = String(p);
      String castToSend = String(adc[p]);
      castToSend.toCharArray(sendOverSerial, 8);
      poi.toCharArray(pos, 2);
      Serial.write(pos);
      Serial.write("~");
      Serial.write(sendOverSerial);
  ...
  if ((stringComplete) && (Serial)) {
    String t1 = serialString.substring(midofA + 1, endofA);
    String t2 = serialString.substring(startofA + 1, midofA);
    int pinNo = t2.toInt();
    int state = t1.toInt();
...
void serialEvent() {
  ...
  while (Serial.available()) {
    ...
    switch (inChar) {
      case '\n':
        endofA = serialStringLength;
        break;
      case '$':
        startofA = serialStringLength;
        break;
      case ',':
        midofA = serialStringLength;
        break;
    }
